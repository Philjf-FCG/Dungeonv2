...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FMyProjectMenuLogicTest,
                                 "MyProject.UI.Menu.Logic",
...
bool FMyProjectMenuLogicTest::RunTest(const FString &Parameters) {
  ...
  UMyDungeonGameInstance *GI =
      NewObject<UMyDungeonGameInstance>(World); // Transient GI
  ...
  if (GI->PartySlots.Num() == 1) {
    TestTrue("Hero is Grog",
             GI->PartySlots[0].HeroName.ToString().Contains("Grog"));
  } else {
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FMyProjectMenuStructureTest,
                                 "MyProject.UI.Menu.Structure",
                                 EAutomationTestFlags::EditorContext |
                                     EAutomationTestFlags::EngineFilter)
...
bool FMyProjectMenuStructureTest::RunTest(const FString &Parameters) {
  ...
  for (const FString &Path : Candidates) {
    ...
    if (BPClass && BPClass->IsChildOf(UW_CharacterSelection::StaticClass())) {
      ...
      UWorld *World = UWorld::CreateWorld(EWorldType::Game, false);
      UW_CharacterSelection *Widget =
          CreateWidget<UW_CharacterSelection>(World, BPClass);
      ...
      if (Widget) {
        ...
        for (UWidget *Child : AllWidgets) {
          if (Child->IsA(UButton::StaticClass())) {
            ButtonCount++;
            bool bVisible =
                (Child->GetVisibility() == ESlateVisibility::Visible);
            AddInfo(FString::Printf(TEXT("  Found Button: %s (Visible: %s)"),
                                    *Child->GetName(),
                                    bVisible ? TEXT("Yes") : TEXT("No")));

            if (!bVisible) {
              AddWarning(FString::Printf(TEXT("  Button %s is hidden! User "
                                              "might not be able to click it."),
                                         *Child->GetName()));
            }
          }
        }
        ...
        if (ButtonCount == 0) {
          ...
          AddInfo(FString::Printf(
              TEXT("  Found %d buttons in %s. This looks like a valid menu."),
              ButtonCount, *Path));
        }
      }
      World->DestroyWorld(false);
  ...
  if (!bFoundAnyValidWidget) {
    AddWarning("Could not find ANY Blueprint Widget inheriting from "
               "UW_CharacterSelection in expected locations.");
  }
